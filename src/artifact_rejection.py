...
def reject_hfo(raw, detector='LineLength',
               threshold=10,
               filter_band=(80, 250),
               win_size=10,
               visualize=True):
    ...
    hfo_raw = _examine_hfo(raw, annot_file, hfo_params, visualize)
    return hfo_raw
...
def _examine_hfo(raw, annot_file, hfo_params, visualize):
    ...
    hfo_annot.rename(dict(zip(hfo_annot.description, map(lambda x: x.replace('HFO at', 'BAD'), hfo_annot.description))))

    raw = raw.set_annotations(raw.annotations + hfo_annot)
...
def extract_gamma_power(epochs):
    ...
    n_cycles = freqs * .1
    gamma_power = mne.time_frequency.tfr_morlet(epochs, freqs=freqs, n_cycles=n_cycles, return_itc=False, use_fft=True, n_jobs=4)
