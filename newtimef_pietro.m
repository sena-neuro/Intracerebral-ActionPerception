...
function [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,maskersp,maskitc,g] = newtimef_pietro( data, frames, tlimits, Fs, varwin, varargin);
...
%      if g.baseline(1)==g.baseline(2);
%             for index = 1:size(g.baseline,1)
%                 tmptime   = find(timesout >= min(timesout) & timesout <= min(timesout)+100);
